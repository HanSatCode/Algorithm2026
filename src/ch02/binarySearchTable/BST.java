package ch02.binarySearchTable;
...
class Node<K, V> {
    ...
    int N; int aux; Node <K, V> parent;

    public Node(K key, V value) { this.key = key; this.value = value; this.N = 1; }
...
public class BST<K extends Comparable<K>, V> {
    ...
    protected void rebalanceInsert(Node<K, V> x) { resetSize(x.parent, 1); }
    protected void rebalanceDelete(Node<K, V> p) { resetSize(p, -1); }
    private void resetSize(Node<K, V> x, int value) { for(; x != null; x = x.parent) { x.N += value; } }
    ...
    private void inorder(Node<K, V> x, ArrayList<K> keyList) {
        if (x != null) { inorder(x.left, keyList); keyList.add(x.key); inorder(x.right, keyList); }
    }
    ...
    public void delete(K key) {
        ...
        if (x == root || isTwoNode(x)) {
            if (isLeaf(x)) { root = null; return; }
                ...
                y = min(x.right); x.key = y.key; x.value = y.value;
                p = y.parent; relink(p, y.right, y == p.left);
                rebalanceDelete(p);
            }
        }
        else {
            ...
            else { relink(p, x.right, x == p.left); }
            rebalanceDelete(p);
        }
    }
